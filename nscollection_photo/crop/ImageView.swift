...
class ImageView: NSImageView {
    ...
    @objc func croppedImageData(_ size: NSSize, compression: Float) -> Data? {
        ...
        if let source = self.image {
            ...
            if let TIFFrepresentation = target.tiffRepresentation {
                ...
                let properties = [NSBitmapImageRep.PropertyKey.compressionFactor:compression]
                imageRepresentation?.converting(to: .sRGB, renderingIntent: NSColorRenderingIntent.perceptual)
    ...
    @objc let cursors =   [NSCursor(image: NSImage(named: NSImage.Name(rawValue: "DiagonalResizeCursor"))!, hotSpot: NSMakePoint(9, 9)),
                     NSCursor(image: NSImage(named: NSImage.Name(rawValue: "BackDiagonalResizeCursor"))!, hotSpot: NSMakePoint(9, 9))]
    ...
    fileprivate func updateCropBox() {
        ...
        overlay.windingRule = .evenOddWindingRule
